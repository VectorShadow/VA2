...
import main.modes.menu.estate.ScrollableItemSelectionMode;
    ...
    protected void handleMenuOptionIndex(int index) {
        switch (index) {
            case MenuDefinitions.LIBRARY_OPTIONS_TEXTS:
                Session.getModeManager().transitionTo(new ScrollableItemSelectionMode(new ResearchTextsSelectionExecutor(Session.getPlayer().getUnresearchedTexts())));
                return;
